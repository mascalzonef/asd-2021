...
j. find all account (from AccountDao) with cf customer code read as parameter [query method AND query AND specification]
k. find all Customer using a filter class whith the follow field:
    [lastname, cf, address, birthdate, acctNo, num of Loan]
    NB. some filter can be null

Extra
A customer with zero loan can has only "Obs Account".
A customer with at leat one loan become automatically "Stardard Account"
